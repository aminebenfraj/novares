...
import { getSafetyById, updateSafety } from "../../../apis/readiness/safetyApi"
...
const fieldConfig = {
  industrialMeansCompliance: {
    label: "Industrial Means Compliance",
    description: "Compliance with industrial safety standards and regulations",
  },
  teamTraining: {
    label: "Team Training",
    description: "Safety training for team members",
  },
  safetyOfficerInformed: {
    label: "Safety Officer Informed",
    description: "Safety officer has been informed and has approved the process",
  },
}
...
function EditSafetyPage() {
  ...
  const [activeTab, setActiveTab] = useState("industrialMeansCompliance")
  ...
  useEffect(() => {
    const fetchSafety = async () => {
      try {
        setIsLoading(true)
        const data = await getSafetyById(params.id)
        setSafety(data)

        // Extract readinessId from the safety object
        ...
        console.error("Error fetching safety record:", error)
        toast({
          variant: "destructive",
          title: "Error",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateSafety(params.id, safety)

      toast({
        title: "Success",
        description: "Safety record updated successfully",
      })
      ...
      if (readinessId) {
        ...
        try {
          ...
          // Find the readiness entry that references this safety record
          const readinessEntry = readinessEntries.find(
            (entry) => entry.Safety === params.id || (entry.Safety && entry.Safety._id === params.id),
          )
        ...
        // Fallback to safety details page if readinessId is not available
        console.log("No readinessId found, navigating to safety detail")
        navigate(`/safety/${params.id}`)
      }
    } catch (error) {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: "Failed to update safety record",
      })
    } finally {
      setIsSaving(false)
    }
        ...
        <div className="container px-4 py-8 mx-auto">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Safety record not found</CardDescription>
            </CardHeader>
            <CardFooter>
              <Button onClick={() => navigate("/safety")}>
                <ArrowLeft className="w-4 h-4 mr-2" /> Back to Safety Records
              </Button>
            </CardFooter>
          </Card>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold tracking-tight">Edit Safety Record</h1>
                <p className="text-muted-foreground">Update safety compliance details and validation information</p>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 gap-6 md:grid-cols-4">
              <Card className="md:col-span-1">
                <CardHeader>
                  <CardTitle>Safety Fields</CardTitle>
                  ...
                  <Tabs defaultValue="status" className="w-full">
                    ...
                    <TabsContent value="status" className="pt-4">
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          id={`${activeTab}-value`}
                          checked={safety[activeTab]?.value || false}
                      ...
                      >
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center space-x-2">
                              <Checkbox
                                id={`${activeTab}-tko`}
                                checked={safety[activeTab]?.details?.tko || false}
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center space-x-2">
                              <Checkbox
                                id={`${activeTab}-ot`}
                                checked={safety[activeTab]?.details?.ot || false}
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center space-x-2">
                              <Checkbox
                                id={`${activeTab}-ot_op`}
                                checked={safety[activeTab]?.details?.ot_op || false}
                                onCheckedChange={(checked) =>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center space-x-2">
                              <Checkbox
                                id={`${activeTab}-is`}
                                checked={safety[activeTab]?.details?.is || false}
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center space-x-2">
                              <Checkbox
                                id={`${activeTab}-sop`}
                                checked={safety[activeTab]?.details?.sop || false}
                                onCheckedChange={(checked) =>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center space-x-2">
                              <Checkbox
                                ...
                                checked={safety[activeTab]?.details?.validation_check || false}
                                onCheckedChange={(checked) =>
                        ...
                        <div className="space-y-4">
                          <div className="space-y-2">
                            ...
                            <RadioGroup
                              value={safety[activeTab]?.details?.ok_nok || ""}
                          ...
                          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div className="space-y-2">
                              ...
                              <Input
                                id={`${activeTab}-who`}
                                value={safety[activeTab]?.details?.who || ""}
                            ...
                            <div className="space-y-2">
                              ...
                              <Input
                                id={`${activeTab}-when`}
                                value={safety[activeTab]?.details?.when || ""}
                          ...
                          <div className="space-y-2">
                            ...
                            <Textarea
                              ...
                              value={safety[activeTab]?.details?.comment || ""}
