...
const CreateOkForLunch = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createOkForLunch(formData)
      toast({
        title: "Success",
        description: "OkForLunch Created!",
      })
      navigate("/okforlunch")
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to create OkForLunch",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
    ...
    <motion.div initial="hidden" animate="visible" variants={formVariants} className="container p-6 mx-auto">
      <Card className="max-w-lg mx-auto shadow-lg">
        <CardHeader className="bg-gray-50">
          <CardTitle className="text-2xl font-semibold text-center text-gray-800">Create OkForLunch</CardTitle>
        </CardHeader>
        <CardContent>
