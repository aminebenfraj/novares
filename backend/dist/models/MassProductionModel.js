...
var massProductionSchema = new Schema({
  id: {
    type: String,
    required: true,
    index: true,
...
massProductionSchema.virtual("days_until_ppap_submission").get(function () {
  ...
  var today = new Date();
  var ppapDate = new Date(this.ppap_submission_date);
  return Math.max(0, Math.ceil((ppapDate - today) / (1000 * 60 * 60 * 24)));
});
massProductionSchema.index({
  id: 1,
  createdAt: -1
});
