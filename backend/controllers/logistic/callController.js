...
exports.exportCallsToExcel = async (req, res) => {
  try {
    ...
    res.setHeader("Content-Disposition", `attachment; filename=llamadas_${new Date().toLocaleString().split("T")[0]}.xlsx`);
...
exports.createCall = async (req, res) => {
  try {
    ...
    // Emit Socket.IO event for new call
    const emitNewCall = req.app.get("emitNewCall");
    if (emitNewCall) {
      await emitNewCall(savedCall);
    } else {
      console.warn("Socket.IO emitNewCall function not available");
    }
...
exports.completeCall = async (req, res) => {
  try {
    ...
    // Emit Socket.IO event for updated call
    const emitNewCall = req.app.get("emitNewCall");
    if (emitNewCall) {
      await emitNewCall(call);
    } else {
      console.warn("Socket.IO emitNewCall function not available");
    }
...
exports.exportCalls = async (req, res) => {
  try {
    ...
    const csvData = [
      ...
      ...calls.map((call) => [
        call.machines.map((machine) => machine.name).join(", "),
  ...
  try {
    ...
    for (const call of pendingCalls) {
      try {
        ...
        const totalSeconds = (call.duration || 90) * 60;
...
exports.deleteCall = async (req, res) => {
  try {
    ...
    // Emit Socket.IO event for deleted call
    const emitNewCall = req.app.get("emitNewCall");
    if (emitNewCall) {
      await emitNewCall({ _id: id, status: "Deleted" });
    } else {
      console.warn("Socket.IO emitNewCall function not available");
    }
